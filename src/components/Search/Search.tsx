...
export default function Search() {
    ...
    // Button va icon ranglarini theme va isActive ga qarab aniqlaymiz
    const isDark = theme === 'dark'
    const buttonBg = isActive
        ? (isDark ? 'bg-white' : 'bg-[#121414]')
        : 'bg-[rgba(131,144,167,0.15)]'
    const buttonHover = isActive ? '' : 'hover:bg-[rgba(131,144,167,0.25)]'
    const buttonTransition = 'transition-all duration-300'
    const buttonClass = `w-[70px] h-[40px] flex items-center justify-center rounded-full ${buttonBg} ${buttonHover} ${buttonTransition}`
    const iconColor = isActive
        ? (isDark ? '#121414' : '#fff')
        : (isDark ? '#fff' : '#8390A7')
    return (
        <div className='max-w-[860px] h-[140px] tabletSm:h-[180px] bg-white p-[25px] tabletSm:p-[20px] mx-auto border border-[rgba(131,144,167,0.3)] rounded-[20px] dark:bg-[#121414] tabletSm:flex tabletSm:flex-col tabletSm:justify-between'>
            <div>
            ...
            <div className='flex justify-between mt-[30px] gap-[10px] tabletSm:mt-[20px]'>
                ...
                >
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 158333V4.16663" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M5 9L10 4L15 9" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                </button>
            </div>
        </div>
